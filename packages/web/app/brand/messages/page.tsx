...
export default function BrandMessagesPage() {
  ...
  const fetchMessages = async (influencerId: string) => {
    try {
      const data = await messagesService.getConversation(influencerId)
  ...
  const searchInfluencers = async (query: string) => {
    ...
    setSearching(true)
    try {
      const data = await influencersService.getAll({ search: query, limit: 10 })
  ...
  const handleSearchChange = (value: string) => {
    ...
    searchInfluencers(value)
  }

  const startConversation = (influencer: Influencer) => {
    setSelectedConversation(influencer.id)
    setSelectedConversationName(influencer.display_name || influencer.user?.email || 'Unknown')
  ...
  const sendMessage = async () => {
    ...
    setSending(true)
    try {
      const data = await messagesService.sendMessage(selectedConversation, 'INFLUENCER', newMessage)
  ...
  const selectedConversationData = conversations.find(c => c.influencer_id === selectedConversation)
  const displayName = selectedConversationData?.influencer_name || selectedConversationName
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Chat with influencers</p>
      </div>
            ...
            {conversations.length === 0 ? (
              ...
              <div className="divide-y">
                {conversations.map((conversation) => (
                  <button
                    key={conversation.influencer_id}
                    onClick={() => {
                      setSelectedConversation(conversation.influencer_id)
                      setSelectedConversationName(conversation.influencer_name)
                    }}
                    className={`w-full p-4 text-left hover:bg-muted/50 transition-colors ${
                      selectedConversation === conversation.influencer_id ? 'bg-muted' : ''
                    }`}
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-1">
                          <h3 className="font-medium truncate">{conversation.influencer_name}</h3>
                    ...
                    >
                      ...
                      <p
                        className={`text-xs mt-1 ${
                          message.sender_type === 'BRAND' ? 'text-primary-foreground/70' : 'text-muted-foreground'
                        }`}
                      >
              ...
              {searching ? (
                ...
                <div className="divide-y">
                  {searchResults.map((influencer) => (
                    <button
                      key={influencer.id}
                      onClick={() => startConversation(influencer)}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-medium truncate">{influencer.display_name || influencer.user?.email}</h4>
                        {influencer.niche && (
                ...
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p>Search for an influencer to start a conversation</p>
                </div>
              )}
            </div>
          </div>
